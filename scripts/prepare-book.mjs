...
function extractWordPages(xml, convertText) {
  const pages = [[]];
  const pageBreakPattern = xml.includes('<w:lastRenderedPageBreak/>') ? /<w:lastRenderedPageBreak\/>/g : /<w:br w:type="page"\/>/g;
  let stanza = [];
  const flushStanza = () => {
  ...
  for (const match of xml.matchAll(/<w:p(?:\s[^>]*)?>([\s\S]*?)<\/w:p>/g)) {
    ...
    segments.forEach((segment, index) => {
      ...
      if (isTitle && text) {
        flushStanza();
        pages.at(-1).push({ text, alignment, isTitle: true });
      } else if (text) {
        stanza.push({ text, alignment });
      } else {
        flushStanza();
      }
...
function splitPagesAtPoemTitles(pages, poemTitles) {
  ...
  for (const sourcePage of pages) {
    ...
    for (const block of sourcePage) {
      ...
      const isPoemTitle = titles.has(normalizeTitle(firstLine));
...
function makeBook({ id, title, source, withContents = false, poemTitles = [], contentsBeforeFirstPoem = false }) {
  const sourcePages = docxPages(source);
  const pages = poemTitles.length ? splitPagesAtPoemTitles(sourcePages, poemTitles) : sourcePages;
  const generatedContents = poemTitles.length > 0;
  ...
  const firstPoemPage = generatedContents
    ? pages.findIndex((blocks) => blocks.some((block) => normalizeTitle(block.text) === normalizeTitle(poemTitles[0])))
    : -1;
  const contents = generatedContents
    ? poemTitles.map((label) => ({
      label,
...
const books = [
  ...
  makeBook({ id: 'prem-o-onnyanno', title: 'প্রেম ও অন্যান্য কবিতা', source: path.join(unicodeFolder, 'প্রেম ও অন্যান্য কবিতা (Unicode).docx') })
];
